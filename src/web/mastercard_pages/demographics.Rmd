...
## Summary Statistics  
The table below shows the summary statistics for different variables relating to demographic information from the American Community Survey (2015-19) Tract level data. We see racially, Arlington County Tracts have about 70% white population on average, with a balanced distribution of other races and ethnic groups. The median household income at a tract-level on average is 130,000, which is much higher than the median household income of an African-American household at a tract level.  
```{r echo=FALSE}
# "totalpop_trct"
...
### One-Way T-Tests  
We also perform one-sample t-tests on our variables to make sure that our variables were continuous, our data were randomly sampled and normally distributed, and the variance was homogenous across variables.  
```{r echo=FALSE}
t_test_tract <- lapply(tract_vars, function(x) 
...
for(x in t_test_tract){
  t_stat = append(t_stat, x$statistic)
  p_value = append(p_value, x$p.value)
  conf_int1 = append(conf_int1, x$conf.int[1])
  conf_int2 = append(conf_int2, x$conf.int[2])
  mean = append(mean, x$estimate)
  std_err = append(std_err, x$stderr)
}

t_test_table_tract <- data.frame(names(t_test_tract), t_stat, p_value, conf_int1, conf_int2, mean, std_err) %>% 
  mutate(mean = round(mean,2),
         std_err = round(std_err, 2),
         conf_int1 = round(conf_int1, 2),
         conf_int2 = round(conf_int2, 2),
         #p_value = round(p_value, 2),
         t_stat = round(t_stat, 2)) %>% 
  select("Statistics" = `names.t_test_tract.`,
         "Mean" = `mean`,
         "Standard Error" = `std_err`,
         "Lower CI (95%)" = conf_int1,
         "Upper CI (95%)" = conf_int2,
         "T-Statistic" = `t_stat`,
         "P Value" = `p_value`) %>% 
  mutate(Statistics = ifelse(Statistics == "totalpop_trct", "Total Population", Statistics),
         Statistics = ifelse(Statistics == "black", "% Population: African-American", Statistics),
...
knitr::kable(t_test_table_tract, format = "html", caption = "Summary Statistics for One-Way T-Tests", align = "c", digits = 50)
```

## Maps  
### Race  
The map below shows the racial demography of Arlington county by census tract while also displaying park space areas. We see that even though tracts 1002 and 1003 are both densely populated, their park access is restricted. Some clusters of areas densely populated with African-American populations do not have good access to parks. We see this especially in Tracts 1031 and 1034. On the other hand, densely populated Asian population areas have tremendous access to parks. We see that access to parks for areas densely populated with Hispanic or Latino populations is fair. Areas densely populated by white persons do not have good access to parks in Arlington County.  
```{r echo=FALSE}
...
leaflet(acs_tract) %>% #create leaflet object
  ...
  addLegend(data = acs_tract %>% drop_na(mean_greenspace), pal = colorNumeric("PuOr", acs_tract$mean_greenspace), values = ~mean_greenspace*100, group = "% Mean Greenspace", title = "% Mean<br>Greenspace", position = "topleft") %>%
   # Layers control
  addLayersControl(
    overlayGroups = c("% Population under 18", "% Pop. with Ambulatory Disability", "% Pop. with No Vehicle", "% Mean Greenspace", "% No Health Insurance", "% Homes with rent > $30K"),
    baseGroups = c("Parks", "No Parks"),
    options = layersControlOptions(collapsed = FALSE)) %>% 
...
### Poverty  
The map below shows the poverty of Arlington county by census tract while also displaying park space areas.  
```{r}
